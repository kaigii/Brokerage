...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] == 'add') {
            ...
            // 檢查員工編號是否重複
            $check = $db->query("SELECT COUNT(*) as count FROM brokers WHERE employee_number = '$employee_number'");
            $row = $check->fetch_assoc();
            if ($row['count'] > 0) {
                echo "<div class='alert alert-danger'>員工編號已存在！</div>";
            } else {
        ...
        if ($_POST['action'] == 'edit') {
            ...
            // 檢查員工編號是否重複（排除自己）
            $check = $db->query("SELECT COUNT(*) as count FROM brokers WHERE employee_number = '$employee_number' AND broker_id != '$broker_id'");
            $row = $check->fetch_assoc();
            if ($row['count'] > 0) {
                echo "<div class='alert alert-danger'>員工編號已存在！</div>";
            } else {
                ...
                if ($db->query($sql)) {
                    echo "<div class='alert alert-success'>更新營業員成功！</div>";
                } else {
